...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  1 10:16:43 2022
...
agruparCedulas = False # Agrupa registros con las mismas cedulas y suma sus valores
getDuplicados = True
...
resagosDF = pd.read_excel('./Hoja de cálculo sin título PV.xlsx', dtype={"CEDULA": str}, skiprows=0)
...
resagosDF["NOMBRES"] = resagosDF["NOMBRES"].str.rstrip(' ').str.lstrip(' ')
resagosDF = limpiarCedula(resagosDF)
resagosDF = limpiarMercado(resagosDF)
resagosDF = limpiarContrato(resagosDF)

duplicados = resagosDF[resagosDF["CEDULA"].duplicated(keep=False)]
resagosDF = resagosDF.drop(duplicados.index)

resagosDF = resagosDF.sort_values('NOMBRES') # Ordena alfabéticamente
resagosDF = resagosDF.reset_index()
...
if (agruparCedulas==True):
    registrosRecuperados = registrosRecuperados.groupby(registrosRecuperados['CEDULA']).aggregate(CEDULA=("CEDULA","first"), NOMBRES=('NOMBRES', 'first'), DESCONTAR=('VALOR A DESCONTAR', 'sum'), VALOR=("VALOR", "first"), REGISTROS_SUMADOS=("MERCADO", "count"), MERCADO=("MERCADO", "first"))
    registrosRecuperados.rename(columns = {'DESCONTAR':'VALOR A DESCONTAR'}, inplace = True)
#============================================================================================================

registrosRestantes = getRestantes(resagosDF, registrosRecuperados)
registrosRestantes = registrosRestantes.reset_index()
