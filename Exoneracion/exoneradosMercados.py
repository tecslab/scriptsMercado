...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 29 11:55:02 2022
...
arrayMercados = [
                 ...
                 {"nombre": "MERCADO 12 ABRIL", "skip": "2",},
...
emptyDF.to_excel('./procesarAparte.xlsx')
...
#for i in range(17,18):
    df = pd.read_excel('./attachments/mercadospq.xlsx', sheet_name=arrayMercados[i]["nombre"], dtype={"CEDULA": str}, skiprows=int(arrayMercados[i]["skip"]))
    print(i)
    print(arrayMercados[i]["nombre"])
    
    registrosFiltradosH3 = registrosConcejalesProcesado[0][registrosConcejalesProcesado[0]["MERCADO"]==arrayMercados[i]["nombre"]]
    registrosFiltradosH5 = registrosConcejalesProcesado[1][registrosConcejalesProcesado[1]["MERCADO"]==arrayMercados[i]["nombre"]]
    ...
    ########## SE GENERA EL EXCEL CON LOS RESULTADOS
    with pd.ExcelWriter('./procesados.xlsx', mode='a', engine="openpyxl") as writer:  
        ...
        todosLosRegistros.to_excel(writer, sheet_name=arrayMercados[i]["nombre"])
    ...
    #     procesarAparte.to_excel(writer, sheet_name=arrayMercados[i]["nombre"])
    ##################################################################################################
    ...
    with pd.ExcelWriter('./duplicados.xlsx', mode='a', engine="openpyxl") as writer:  
        duplicados.to_excel(writer, sheet_name=arrayMercados[i]["nombre"])
    ##################################################################################################
...
#df = pd.read_excel('./attachments/arenal.xlsx', sheet_name="SECTOR JUNTO A LAS NAVES DEL CE", dtype={"CEDULA": str}, skiprows=2)
